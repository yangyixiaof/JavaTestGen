...
public class InfoOfBranch {
	...
	public void HandleOneValueOfBranch(ValuesOfBranch vob) {
		...
		state = SimpleInfluenceComputer.MergeBranchState(vob.GetCmpOptr(), one_state, state);
	...
	// for a state, if mutating a position causes transferring, we must see how many branches it influences to decide the weights
	// for a state, we could record how many states are transfered to through mutating a position
