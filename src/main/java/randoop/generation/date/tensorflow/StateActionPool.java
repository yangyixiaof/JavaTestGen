...
public class StateActionPool {
	...
	Map<LinkedSequence, ArrayList<MutationOperation>> state_actions = new TreeMap<>();
	Map<LinkedSequence, ArrayList<Integer>> state_action_states = new TreeMap<>();// 1 means taken, 0 means un_taken
	Map<LinkedSequence, Integer> state_untaken_actions = new TreeMap<>();
	
	// correspond to state_actions
	Map<LinkedSequence, ArrayList<TreeMap<String, Double>>> all_accumulated_influences = new TreeMap<>();
	 ...
	 * @return
	 */
	public ArrayList<MutationOperation> GetAllActionsOfOneState(LinkedSequence state) {
	...
	public void ActionOfOneStateBeTaken(LinkedSequence state, int action_index) {
		state_action_states.get(state).set(action_index, 1);
		state_untaken_actions.put(state, state_untaken_actions.get(state)-1);
	}
	
	public boolean DoNotHaveUntakenActionsOfOneState(LinkedSequence state) {
		return state_untaken_actions.get(state) <= 0;
	}
	
	public ArrayList<MutationOperation> GetUntakenActionsOfOneState(LinkedSequence state) {
	...
	public void BackTraceToStoreDiscountedInfluence(QTransition transition, Map<String, Double> current_influences, int back_turns) {
		...
		LinkedSequence source_sequence = transition.GetSourceSequence();
