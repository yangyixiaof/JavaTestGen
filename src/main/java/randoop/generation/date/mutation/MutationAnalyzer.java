...
public class MutationAnalyzer {
  ...
  private void RecursiveToGenerateInsertOperations(
      int stmt_idx,
      TypedOperation optr,
      List<MutationOperation> mutates,
      List<LinkedList<Variable>> suitable,
      int index1,
      Stack<Variable> history) {
  ...
  public void GenerateModifyPrimitiveIntegralOperations(List<MutationOperation> mutates) {
    ...
    while (tsfi.HasNext()) {
      ...
      for (int it = 0; it < it_len; it++) {
        Type tp = input_types.get(it);
        if (tp.isPrimitive()
            && (tp.getCanonicalName().equals("java.lang.Character")
                || tp.getCanonicalName().equals("char")
                || tp.getCanonicalName().equals("java.lang.Byte")
                || tp.getCanonicalName().equals("byte")
                || tp.getCanonicalName().equals("java.lang.Short")
                || tp.getCanonicalName().equals("short")
                || tp.getCanonicalName().equals("java.lang.Integer")
                || tp.getCanonicalName().equals("int")
                || tp.getCanonicalName().equals("java.lang.Long")
  ...
  public void GenerateModifyPrimitiveRealOperations(List<MutationOperation> mutates) {
    ...
    while (tsfi.HasNext()) {
      ...
      for (int it = 0; it < it_len; it++) {
        Type tp = input_types.get(it);
        if (tp.isPrimitive()
            && (tp.getCanonicalName().equals("float")
                || tp.getCanonicalName().equals("Float")
                || tp.getCanonicalName().equals("double")
                || tp.getCanonicalName().equals("Double"))) {
  ...
  public void GenerateModifyPrimitiveBooleanOperations(List<MutationOperation> mutates) {
    ...
    while (tsfi.HasNext()) {
      ...
      for (int it = 0; it < it_len; it++) {
        Type tp = input_types.get(it);
        if (tp.isPrimitive()
            && (tp.getCanonicalName().equals("java.lang.Boolean")
        ...
        if (tp.isReferenceType()) {
          if (tp != null) {
            if (tp.getCanonicalName().equals("java.lang.String")) {
              ...
              Statement input_generate_statement =
                  sequence.getStatement(input_generate_statement_index);
